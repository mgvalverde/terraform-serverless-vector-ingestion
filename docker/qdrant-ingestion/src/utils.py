...
from typing import Dict, Iterable, List, Any, Union, Annotated
...
def batch_generator(iterator, batch_size=100):
    batch = []
...
@lru_cache()
def get_index_from_vector_store(
        client,
        collection_name,
        enable_hybrid=True,
        fastembed_sparse_model=None
):
    if enable_hybrid and fastembed_sparse_model is None:
        raise ValueError("If 'enable_hybrid=True', provide 'fastembed_sparse_model=fastembed_sparse_model'")
...
def force_uuid(id_:Union[UUID, str]):
    if isinstance(id_, str):
        return UUID(id_)
    return id_

class NamespaceUUIDTransform(TransformComponent):

    namespace: Annotated[Union[UUID, str], BeforeValidator(force_uuid)] = Field(
        description="UUID Namespace to provide consisting UUID",
    )

    def __call__(self, nodes, **kwargs):
        for node in nodes:
            node.id_ = str(uuid5(namespace=self.namespace, name=node.text))
        return nodes
