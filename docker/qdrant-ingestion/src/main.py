...
from utils import get_parameter, extract_event, extract_path_component, get_index_from_vector_store, NamepaceUUIDTransform
...
SQS_ACK_QUEUE_URL = os.environ["SQS_ACK_QUEUE_URL"]
...
jina_api_ssm = os.environ["JINA_API_SSM"]
qdrant_api_ssm = os.environ["QDRANT_API_SSM"]
qdrant_url_ssm = os.environ["QDRANT_URL_SSM"]
namespace_uuid_ssm = os.environ["NAMESPACE_UUID_SSM"]
...
namespace_uuid = get_parameter(name=namespace_uuid_ssm)
...
def handler(event, context):
    ...
    for ith, message in enumerate(messages, 1):
        ...
        try:
            ...
            # )
            index = get_index_from_vector_store(
                client=qdrant,
                collection_name=collection_name,
                enable_hybrid=True,
                fastembed_sparse_model=SPARSE_EMBED_MODEL_NAME
            ...
            pipeline = IngestionPipeline(
                transformations=[
                    ...
                    embed_model,
                    NamepaceUUIDTransform(namespace=namespace_uuid)
                ],
                # vector_store=vector_store
            )
            ...
            index.insert_nodes(
                nodes=nodes,
                use_async=False  # NOTE: Lambda fails if async = True for Qdrant
            )

            logger.info("Loaded %s nodes in %s", len(nodes), collection_name)
